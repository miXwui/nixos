# Do not modify this file!  It was generated by ‘nixos-generate-config’
# and may be overwritten by future invocations.  Please make changes
# to /etc/nixos/configuration.nix instead.

{
  config,
  lib,
  pkgs,
  modulesPath,
  ...
}:

{
  imports = [ (modulesPath + "/installer/scan/not-detected.nix") ];

  # TODO: regeneration with `nixos-generate-config` and/or check
  # settings in nixos-hardware repo:
  # boot.initrd.availableKernelModules = [ "nvme" "xhci_pci" "thunderbolt" "usb_storage" "sd_mod" ];
  # boot.initrd.kernelModules = [ ];
  # boot.kernelModules = [ "kvm-intel" ];
  # boot.extraModulePackages = [ ];

  # TODO: test power savings
  # # https://github.com/NixOS/nixpkgs/issues/211345#issuecomment-1387439246
  # boot.kernelParams = [
  #   "ahci.mobile_lpm_policy=3"
  # ];

  boot.loader.systemd-boot.extraEntries."fedora.conf" = ''
    title Fedora
    efi   /efi/fedora/grubx64.efi
  '';

  boot.loader.systemd-boot.extraEntries."ubuntu.conf" = ''
    title Ubuntu
    efi   /efi/ubuntu/grubx64.efi
  '';

  fileSystems."/" = {
    device = "UUID=d537ef7f-1f95-428e-9ad1-e30965bef078";
    fsType = "bcachefs";
  };

  fileSystems."/boot" = {
    device = "/dev/disk/by-uuid/FDA6-7EC1";
    fsType = "vfat";
    options = [
      "fmask=0022"
      "dmask=0022"
    ];
  };

  swapDevices = [ ];

  # Enables DHCP on each ethernet and wireless interface. In case of scripted networking
  # (the default) this is the recommended approach. When using systemd-networkd it's
  # still possible to use this option, but it's recommended to use it in conjunction
  # with explicit per-interface declarations with `networking.interfaces.<interface>.useDHCP`.
  networking.useDHCP = lib.mkDefault true;
  # networking.interfaces.enp195s0f3u1u6.useDHCP = lib.mkDefault true;
  networking.hostName = "nixos-framework-13-intel-i7-1165g7";

  nixpkgs.hostPlatform = lib.mkDefault "x86_64-linux";
  hardware.cpu.intel.updateMicrocode = lib.mkDefault config.hardware.enableRedistributableFirmware;
}
